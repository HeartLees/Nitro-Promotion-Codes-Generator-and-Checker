    ...
    def main(self):
        ...
        if os.name == "nt":
            ...
            ctypes.windll.kernel32.SetConsoleTitleW(
                "Nitro Promotion Codes Generator and Checker - Made by ! Desprate ")
        else:  # Or if it is unix
            print(f'\33]0;Nitro Promotion Codes Generator and Checker - Made by Desprate\a',
                  end='', flush=True) 
...
""")
        ...
        for s in c:
            ...
            if os.name == "nt":
                ctypes.windll.kernel32.SetConsoleTitleW(
                    f"Nitro Promotion Codes Generator and Checker - {len(valid)} Valid | {invalid} Invalid - Made by Desprate")
                print("")
            else:
                print(
                    f'\33]0;Nitro Promotion Codes Generator and Checker - {len(valid)} Valid | {invalid} Invalid - Made by Desprate\a', end='', flush=True)
    ...
    def quickChecker(self, nitro:str, notify=None):
        ...
        if response.status_code == 200:
            ...
            if notify is not None:
                DiscordWebhook(
                    url=url,
                    content=f"تم الحصول على رابط متاح @here \n{nitro}"
                ).execute()
